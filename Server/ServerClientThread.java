...
public class ServerClientThread extends Thread {
        ...
        if (userId != null) {
            ...
            if (userInfo.length > 1) {
                ...
                if (foundAcc || userInfo[1].equals("")) {
                    if (userInfo.length < 3 || passwordAccepted || isLoggedIn) {
                        status = STATUS_LOGGEDIN;
                        isLoggedIn = true;
            ...
            status = STATUS_ERROR;
            serverMsg = "Please specify a user first with the 'USER [user_id]' command.";
        }
    ...
    public void LIST(String list_cmd) throws IOException {
        if (isLoggedIn) {
            ...
            if (dirToList.exists()) {
                ...
                status = STATUS_ERROR;
                serverMsg = dirToList.getCanonicalPath() + " does not exist. Please try again.";
            }
        }
        else {
            status = STATUS_ERROR;
    ...
    public void CDIR(String new_dir) throws IOException {
        if (isLoggedIn) {
            ...
            Path newDirPath = Paths.get(new_dir);
